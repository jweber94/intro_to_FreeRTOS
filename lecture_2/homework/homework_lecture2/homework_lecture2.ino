...
// blinky times
uint32_t* time1 = new uint32_t(500);
uint32_t* time2 = new uint32_t(323);

/* functions to use in the main setup and loop */
void toggleLED(void* parameter) {
  uint32_t* sleepTimeMs = (uint32_t*) parameter;
  while(1) {
...
void setup() {
  ...
  // create task to run the first blinky function
  xTaskCreatePinnedToCore(
    toggleLED,      // function with signature BaseType_t (*TaskHookFunction_t)( void * ) that contains the task execution instructions 
    "ToggleLED1",   // name of the task
    1024,           // stack size
    (void*)time1,  // parameters to the function (needs to be casted as void*)
    1,              // task priority
  ...
  // create task to run the second blinky function to overlay the first blinky
  
  xTaskCreatePinnedToCore(
    toggleLED,      // function with signature BaseType_t (*TaskHookFunction_t)( void * ) that contains the task execution instructions 
    "ToggleLED2",   // name of the task
    1024,           // stack size
    (void*)time2,  // parameters to the function (needs to be casted as void*)
    1,              // task priority
    NULL,           // task handle - this can be handed over to other task to check this tasks status, memory usage etc from other tasks
    app_cpu         // core to pin the task to (only esp32 specific/espressive FreeRTOS)
  );
